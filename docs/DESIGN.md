# Marque — Design Notes & Measurements

The detailed companion to the [README](../README.md): how verification, tier 3, and
retrieval actually work, with the measured numbers behind the headline claims.
...
baseline, which matches meaning rather than words, leads in absolute terms — though
a paired significance test finds the gap statistically indistinguishable on a single
run (McNemar *p* = 0.15):
...
See [`bench/financebench/README.md`](../bench/financebench/README.md) for the
McNemar significance tests, confidence intervals, and run-to-run variance.
...
- The query-cost benchmark (the PageIndex column above) models payload size and
  ...
  PageIndex run. Marque's own side is measured live — see the Tier 3 and
